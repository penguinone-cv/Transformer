...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, heads, d_ff, dropout=0.1):
        ...
        self.attention_models = nn.Sequential()
        self.heads = heads
        for i in range(heads):
            name = "Scaled-Dot Product Attention" + str(i)
            self.attention_models.add_module(name, Attention(d_model))
    ...
    def forward(self, x, mask):
        ...
        for i in range(self.heads):
            output, _ = self.attention_models.layer[i](x_normlized, x_normlized, x_normlized, mask)
            attentions.append()
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        attentions = attentions.to(device)
        output = torch.cat(attentions)
...
class TransformerClassification(nn.Module):
    ...
    def __init__(self, text_embedding_vectors, heads, layers_num, dropout_rate=0.1, d_model=300, d_ff=1024, max_seq_len=256, output_dim=2):
        super().__init__()
        ...
        for i in range(layers_num):
            name = "Encoder" + str(i)
            self.encoders.add_module(name ,TransformerBlock(d_model, heads, d_ff, dropout_rate))
    ...
    def forward(self, x, mask):
        ...
        for i in range(self.layers_num):
            x, _ = self.encoders.layer[i](x, mask)
